...
export function createServer({ dbPath, vaultKey, origins = ['http://127.0.0.1:5188'], network = 'local', rpc, production = false, chain: injectedChain, now = Date.now } = {}) {
  if (!['local', 'devnet'].includes(network)) throw new Error('Only local and devnet are supported. Mainnet is not implemented.');
  ...
  const chain = injectedChain || (network === 'devnet' ? devnetChain(rpc) : null);
  ...
  app.get('/api/health', (req, res) => res.json({ ok: true, service: 'tekkwork-api', network, mainnet: false, trading: false }));
  app.get('/api/state', (req, res) => {
    const session = getSession(req);
    const agents = session ? db.prepare('SELECT * FROM agents WHERE owner=? ORDER BY no DESC').all(session.address).map(rowAgent) : [];
  ...
  app.get('/api/agents/:id', auth, (req, res) => res.json(owned(req).agent));
  app.patch('/api/agents/:id', auth, (req, res) => {
    ...
    a.strategy = req.body.strategy; a.updatedAt = now(); save(a); event(a, 'settings', `${a.name}'s strategy profile was updated. Execution remains disabled.`); res.json(a);
  });
  ...
  app.post('/api/agents/:id/prepare', auth, locked(async (req, res, row, a) => {
    ...
    if (!a.launch || a.launch.expiresAt <= now() || a.status === 'FAILED') {
  ...
  async function reconcileAgent(a) {
    if (chain && a.status === 'SUBMITTED') {
      const status = await chain.status(a.launch.signature,a.launch.lastValidBlockHeight);
  ...
  app.post('/api/agents/:id/reconcile', auth, locked(async (req, res, row, a) => {
    await reconcileAgent(a);
    res.json(a);
  }));
  let workerRunning=false;
  const reconcilePending=async()=>{
    if(!chain||workerRunning)return;
    workerRunning=true;
  ...
  app.use((err, req, res, next) => {
    ...
    if (status >= 500) console.error('API operation failed:', err.name);
  ...
  return { app, close: store.close, store, reconcilePending };
}
